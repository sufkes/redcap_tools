...
fields_normal = ['mridate', 'mritime', 'scn_strgth', 'oth_strgth', 'seq_acq', 'specify', 'mri_satisfact', 'acuteinfarct_pres', 'les_cnt', 'lesionvol', 'lesionseq', 'surf_area', 'infarct_thick', 'infarct_vol', 'brain_vol', 'inf_vol_relative', 'subcort_infarct', 'mca_inv', 'grwm_tracts', 'hem', 'cort_rd', 'cort_edem', 'ad_find', 'vas_imag_perf', 'vas_imag', 'anat_var', 'abn_obs']
fields_groups = [['ant_chor___1', 'ant_cer___1', 'mca___1', 'pca___1', 'vertb_vasc___1', 'watshed___1'], ['mrab_date', 'mran_date', 'ctab_date', 'ctan_date', 'dsa_date']]
...
name = "neuroimaging_fields"
description = "Missing fields in Neuroimaging form"
report_forms = True
inter_project = False
...
def fieldConditions(field_name, metadata):
    ...
    if field_name.split('___')[0] in fields_normal: # look for the base name of the checkbox field in the requested field list.
        return True
...
def checkFunction(row_index, field_name, def_field, metadata, records, record_id_map, repeating_forms_events, form_repetition_map):
    ...
    if field_name.split('___')[0] in fields_normal: # if in the normal field list    
