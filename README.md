...
An instrument's completion status can be set to `0`/`Incomplete` (red), `1`/`Unverified` (yellow), or `2`/`Complete` (green), even if the instrument contains no data. In such cases, it is often desired to set the completion status to blank. This script finds all empty forms, and replaces their completion statuses with blanks. The user may also generate a CSV file which can be manually imported to overwrite the completion statuses blank (safer; recommended).
