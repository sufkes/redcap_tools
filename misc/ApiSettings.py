...
#            pass
#        else:
#            raise ValueError("num_args must be an integer greater than 0."
        if (num_projects == 1):
            project_id_group.add_argument("-n", "--code_name", help="Code name of project in user's api_keys.json file.")
            ...
            project_id_group.add_argument("-n", "--code_names", help="Code names of projects in user's api_keys.json file.", nargs=num_projects, metavar='CODE_NAME', default=[None]*num_projects)
    ...
    def getApiCredentials(self, api_url=None, api_key=None, code_name=None, api_key_path=None):
        ## Get settings
        # If a custom api_key_path was not specified, use the path stored in settings.json
        ...
        if ((not code_name is None) and (api_key_path is None)):
            raise Exception("If a project ID is specified, must also specify a path to a json file which maps project IDs to API URLS and tokens. See the example in '"+os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'redcap_directory_template', 'api_keys.json'))+"'")

        if ((not code_name is None) and (not api_key_path is None)):
            if (not os.path.exists(api_key_path)):
                raise Excpetion(msg_api_key_path_not_found)
            with open(api_key_path, 'r') as handle:
                api_map = json.load(handle)
            try:
